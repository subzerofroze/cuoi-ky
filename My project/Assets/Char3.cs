...
public class Char3 : MonoBehaviour
{
    ...
    public string GetName()
    {
        ...
        GameObject Character3 = new GameObject("Char3Name");

        // Đặt tên cho GameObject thông qua thuộc tính name
        Character3.name = "Kurt";

        return Character3.name;
    }
    ...
    private IEnumerator EncounterAnimation()
    {
        ...
        switch (selectedAnimation)
        {
            ...
            case 4:
                ...
                Char3position.x = xPosition + distanceTraveled;
                Rchar3.position = Char3position; // Update the character position
                MoveCharacter(Rchar3); // Start the race again
                yield break;
        ...
        if (!hasEncounteredAnimation2)
        {
            ...
            if (elapsedTime2 >= 25f)
            {
                ...
                if (randomValue2 >= 90)
                {
                    ...
                    Transform Char3transform = transform;
                    Vector2 Char3position = Char3transform.position;
                    Char3position.x = finishLine.position.x; //Nhảy đến vạch đích và dừng chuyển động
                    Rchar3.position = Char3position;
                    StopMovement(Rchar3);
                }
            }
        }
